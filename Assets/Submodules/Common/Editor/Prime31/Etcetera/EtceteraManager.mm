...
- (void)startCameraCaptureWithFrontFacingCamera:(bool)useFrontCamera previewFrame:(CGRect)frame outputFileURL:(NSURL*)outputFileURL
{
	...
	CALayer* viewLayer = UnityGetGLView().layer;
	...
	{
		// one day, when Apple fixes the bugs with AVCaptureController.videoOrientation swtich this over to use it
...
{
	NSLog( @"recording completed. error: %@", error );
	if( error )
